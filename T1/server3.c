...
//S3: servidor concorrente sob demanda com processos: A cada requisição, o servidor gera um novo processo para atendimento de uma única requisição do cliente. O novo processo fecha a conexão depois de atender uma requisição e finaliza execução.
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <errno.h>
...
// fazer bind para o primeiro que pudermos 
  ...
  while(1) { // loop principal de accept() 
    ...
    // para cada request, um novo processo para atender essa requisicao. O novo processo fecha a conexão depois de atender uma requisição e finaliza execução.
